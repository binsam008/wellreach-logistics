...
export default function BillingPanel() {
  ...
  async function openPdf(inv) {
    try {
      ...
      const message = err?.response?.data?.error || err?.message || "Failed to open PDF";
  ...
  async function doAddPayment() {
    ...
    try {
      setPayLoading(true);
      const res = await api.put(`/invoices/${paymentInvoice._id}/pay`, { amount: amt });
      if (res?.data?._id) {
        setInvoices((prev) => prev.map((p) => (p._id === res.data._id ? res.data : p)));
      } else {
        await fetchInvoices();
      }
  ...
  function paymentStatus(inv) {
    ...
    if (total <= 0) return { label: "UNBILLED", classes: "bg-slate-100 text-slate-800" };
    if (paid <= 0) return { label: "UNPAID", classes: "bg-rose-100 text-rose-700" };
    if (paid < total) return { label: "PARTIAL", classes: "bg-amber-100 text-amber-700" };
      ...
      <div className="card">
        <div className="card-inner flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500">Review invoices, record payments and export PDFs for clients.</p>
          </div>

          <div className="flex items-center gap-3">
            <div className="hidden md:block">
              <input className="input" placeholder="Search invoice / job / client" />
            </div>
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {invoices.map((inv) => {
                ...
                const taxPercent = inv.taxPercent != null ? inv.taxPercent : deriveTaxFromCountry(inv.country);
                return (
                  <article key={inv._id} className="border rounded-2xl p-4 md:p-5 bg-white flex flex-col gap-4">
                    <div>
                      ...
                      <div className="text-sm text-slate-500 mt-1">
                        ...
                        <div className="text-xs text-slate-400 mt-1">{inv.createdAt ? new Date(inv.createdAt).toLocaleDateString() : ""}</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm text-slate-700">
                      <div>
                        ...
                        <div className="font-semibold">{(inv.total ?? 0).toFixed(3)} {inv.currency}</div>
                      </div>

                      <div>
                        <div className="text-xs text-slate-500">Paid</div>
                        <div className="font-semibold">{(inv.paidAmount ?? 0).toFixed(3)} {inv.currency}</div>
                      </div>

                      <div>
                        <div className="text-xs text-slate-500">Balance</div>
                        <div className="font-semibold">{(inv.balance ?? 0).toFixed(3)} {inv.currency}</div>
                      </div>
{/* 
                      <div>
                    ...
                    <div className={`px-3 py-1 rounded-full w-fit text-xs font-semibold ${status.classes}`}>
                      {status.label}
                    </div>

                    <div className="flex flex-wrap gap-2 mt-2">
                      <button onClick={() => openAddPayment(inv)} className="btn btn-ghost text-xs px-3 py-1.5 flex items-center gap-2">
                        <CreditCard className="w-4 h-4" /> Add Payment
                      </button>

                      <button onClick={() => openEditCharges(inv)} className="btn btn-outline text-xs px-3 py-1.5">
                        <PlusCircle className="w-4 h-4" /> Edit Charges
                      </button>

                      <button onClick={() => openPdf(inv)} className="btn btn-outline text-xs px-3 py-1.5">
                        <FileText className="w-4 h-4" /> PDF
                      </button>

                      <button onClick={() => askDelete(inv)} className="btn text-xs px-3 py-1.5 text-rose-600 border">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-5">
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => setPaymentOpen(false)} className="p-2 rounded-md">✕</button>
            </div>

            <div className="mt-4">
              <p className="text-sm text-slate-600">Balance due: <strong>{(paymentInvoice.balance ?? 0).toFixed(3)} {paymentInvoice.currency}</strong></p>
              <label className="block text-sm mt-3">Amount</label>
              <input type="number" step="0.001" className="input mt-1" value={payAmount} onChange={(e) => setPayAmount(e.target.value)} />
            </div>

            <div className="mt-4 flex justify-end gap-2">
              <button className="btn btn-outline" onClick={() => setPaymentOpen(false)} disabled={payLoading}>Cancel</button>
              <button className="btn btn-primary" onClick={doAddPayment} disabled={payLoading}>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-5">
            <h3 className="text-lg font-semibold">Delete invoice {confirmDelete.invoiceNumber}?</h3>
